...
const ReactCSV = () => {
  ...
  useEffect(() => {
    fetch('Melbourne_housing_FULL.csv')
  ...
  const priceRangeData = data.reduce((acc, curr) => {
    ...
    if (price) {
      const range = Math.floor(price / 500000) * 500000;
      const label = `$${(range / 1000000).toFixed(1)}M - $${((range + 500000) / 1000000).toFixed(1)}M`;
      ...
      <div className="space-y-4">
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex space-x-4 mb-4">
            {['distribution', 'prices', 'rooms', 'correlation'].map((tab) => (
              <button
                key={tab}
          ...
          {activeTab === 'distribution' && (  //Distribution PAGE
          ...
          {activeTab === 'correlation' && ( //Correlation PAGE
            <div className="bg-white rounded-lg p-4">
